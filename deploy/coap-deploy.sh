...
# If the target environment is prod or devHeroku, we use the existing apps coap-silo and dev-coap-silo.
# If it's a canary deploy we create a new app named after the branch to deploy.
#
# Script invocation example:
#   PROD: `coap-deploy.sh -a api-unik-mtl -e prod`
#   DEV: `coap-deploy.sh -a dev-api-unik-mtl -e devheroku`
#   CANARY: `coap-deploy.sh -a canary-test-feature -e canary`  (assuming the branch name to deploy as a canary is canary-test-feature)
#
...
# Delete an existing coap-deploy folder from a previous deploy
echo "Delete existing coap-deploy folder..."
rm -rf ../coap-deploy
# Creating a new coap-deploy folder to isolate the express server code
echo "Create a new coap-deploy folder..."
mkdir ../coap-deploy
# Copy the express server code to coap-deploy
echo "Copy coap server code to coap-deploy..."
cp -a coap-server/. ../coap-deploy
cd ../coap-deploy
...
git init
git add -A
git commit -m "Ready to deploy Silo CoAP Server"
...
echo "Silo CoAP Server Deployed !!"